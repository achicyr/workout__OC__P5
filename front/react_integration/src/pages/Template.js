...
export default class Template{
        ...
        div.innerHTML = `
            <article class="cart__item" data-id="${_id}" data-color="${color}" data-match="${_id+color}">
                <div class="cart__item__img">
                    <img src="../images/${imageUrl}" alt="${altTxt}">
                </div>
    ...
    cutCartProduct = (e,ls,id,orderIndex) => {
        ...
        , doDelete = document.confirm(`Êtes-vous sûr de vouloir supprimer l'article suivant:
            ${ls[id].name} - ${color} ?
        `)
        ...
        if(doDelete){
            ...
            document.getElementById('cart__items').removeChild(e.target.closest('article'))
            this.resetCartTotal()
        }
    }
    /**
    ...
    resetCartTotal = () => {
        ...
        document.getElementById('totalQuantity').innerHTML = cptTotal
    ...
    sendForm = (e,endpoint) => {
        ...
        .then(confirmationOrder => {
            ...
            document.location = 'confirmation.html?orderId=' + confirmationOrder.orderId
        })
        .catch(() => { 
            alert('ok')
            let confirmationOrder = "iJuHYYG767uh87Gè-f76yuiu_78"
            console.log();
            document.location = 'confirmation/orderId=' + confirmationOrder
         })
    }
}